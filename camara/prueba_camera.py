    ...
    if ballPixel == 0:
        ...
        if  ballPixel < (leftBound):
            ...
            if radius > 50 and ballPixel < 110:
                print (ballPixel)
                ser.write(defaultSpeed, 20, 0)
            else:
                ser.write(turnPwm, defaultSpeed, 0)
        elif ballPixel > (rightBound):
            #print "right side"
            print("derecha")
            if radius > 50 and ballPixel > 540:
                print (ballPixel)
                ser.write(20, defaultSpeed, 0)
            else:
                ser.write(defaultSpeed, turnPwm, 0)
        else:
        #print "middle"
            print("medio")
            if (radius < 40):
                ser.write(defaultSpeed, defaultSpeed, 0)
            else:
                pwmStop()
